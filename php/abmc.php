...
function star($operacion, $dato)
{


    function registrastar($dato)
    {
        $userid = $_SESSION['id'];
        $cant = $dato;
        $courseid = $_POST['courseid'];
        $sql = MySQLDB::getInstance()->query("SELECT * FROM starscourse WHERE  userid='$userid' AND courseid='$courseid'");

        if ($sql->num_rows != 0) {


            $sql = MySQLDB::getInstance()->query("UPDATE starscourse SET cant='$cant',date=now() WHERE  userid='$userid' AND courseid='$courseid'");
            consultastar();
            die;
        } else {
            //realizamos el insert del nuevo tema   

            $sql = MySQLDB::getInstance()->query("INSERT INTO starscourse (userid,courseid,cant,date) VALUES ('$userid','$courseid','$cant',now())");
            consultastar();
            die;
        }
    }

    function consultastar()
    {
        $courseid = $_POST['courseid'];
        $sql1 = MySQLDB::getInstance()->query("SELECT * FROM starscourse WHERE courseid='$courseid'");
        $cant = $sql1->num_rows;
        $aux = 0;
        if ($sql1->num_rows != 0) {
...
function perfil($operacion, $dato)
{
    ...
    function modificaperfil($dato)
    {
        ...
        if ($dato == "contraseña") {
            ...
            if (isNull($_POST['nombre'], $_POST['apellido'], $_POST['country'],  $_POST['dni'], $_POST['fecha'])) {
                echo 4;
                die;
            }
            ...
            if (MySQLDB::getInstance()->query("SELECT * FROM countries WHERE nombre='$country'")->num_rows == 0) {
                echo 7; //pais no exite
                die;
            }else{
            $paisquery=MySQLDB::getInstance()->query("SELECT * FROM countries WHERE nombre='$country'");
            $paisq = $paisquery->fetch_assoc();
            $countryid=$paisq['id'];
            }
            $dni = trim(filter_var($_POST['dni'], FILTER_SANITIZE_STRING));
            $d = trim(filter_var($_POST['fecha'], FILTER_SANITIZE_STRING));
            $date = date("Y/m/d", strtotime($d));
            $q2 = MySQLDB::getInstance()->query("UPDATE users SET name='$name', lastname='$lastname', idcountry='$countryid',dni='$dni', date_birth='$date' WHERE id = '$userid'");
            if ($q2) {
                echo 1;
                die;
            } else {
