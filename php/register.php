...
if (isset($_POST['name']) && isset($_POST['lastname']) && isset($_POST['email']) && isset($_POST['pass']) && isset($_POST['rePass'])  && isset($_POST['pais']) && isset($_POST['date'])) {
    $name = trim(filter_var($_POST['name'], FILTER_SANITIZE_STRING));
    $lastname = trim(filter_var($_POST['lastname'], FILTER_SANITIZE_STRING));
    $pais = trim(filter_var($_POST['pais'], FILTER_SANITIZE_STRING));
...
if ($valido) {
    ...
    if (!($name || $lastname || $email || $pass)) { //Filtrado de variables
    ...
    if ($captcha_success->success == false) {
        ...
        if (MySQLDB::getInstance()->query("SELECT * FROM countries WHERE nombre='$pais'")->num_rows == 0) {
            echo 5; //pais no exite
            die;
        } else {
            $paisquery = MySQLDB::getInstance()->query("SELECT * FROM countries WHERE nombre='$pais'");
            $paisq = $paisquery->fetch_assoc();
            $paisid = $paisq['id'];
        }

        $sql = MySQLDB::getInstance()->query("INSERT INTO users (name, lastname, idcountry, date_birth, username ,password,active,token,creation_date) VALUES ('$name', '$lastname','$paisid','$dateb', '$email' ,'$pass_hash','$active','$token',now())");
