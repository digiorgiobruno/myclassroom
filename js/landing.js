...
$('#register_button').click(() => {
    ...
    let pais = $('#localidadAutocomplete').val();
    ...
    if (!is_ok(name)) {
        ...
        if (!is_ok(lastname)) {
            ...
            if (pais.length < 0) {
                $('#validate_register').empty().append('<div class="alert alert-warning" role="alert">El campo país se encuentra vacío, o su valor es incorrecto</div>');
            } else {
                $('#validate_register').empty();
    ...
    if (validaPassword(pass, rePass) && is_ok(name) && is_ok(lastname) && IsEmail(email) && is_ok(pass) && pass.length > 6 &&pais.length>0) {
        $.ajax({
            method: 'POST',
            ...
            success: function (rs) {
                ...
                if (rs == 5) {
                    $('#validate_register').empty().append('<div class="alert alert-warning" role="alert">¡revise el campo país!</div>');
